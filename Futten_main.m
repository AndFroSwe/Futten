...
% Test trajectories for different H
figure ()
count = 1;                              % Set counter
plotStyle = {'b','g','r','m'};          % Set colors of trajectory plots
for H = [10, 8, 6, 2]                   % Test H:s
    [u_t, t_pass, r_pass, phi_pass] = RKeval(h, H, c1);     % Evaluate trajecory with RK4
    H_list = [H_list H];                % Gather used H in list
    t_list = [t_list t_pass];           % Gather used t in list
    r_list = [r_list r_pass];           % Gather used r in list
    phi_list = [phi_list phi_pass];     % Gather used phi in list
    info = [H_list' r_list' phi_list' t_list'];   % Create matrix of end values
    polar(u_t(:,3),u_t(:,1),plotStyle{count})                      % Plot tracectory
...
info = sortrows(info,1);
disp('    H         r         phi       t')    % Display results
disp(info)
